...
class PointNetTransformerRegressor(nn.Module):
    def __init__(self, input_dim=9, point_feat_dim=256, trans_hidden=256, nhead=8, num_layers=4):
        super().__init__()
...
def extract_peak_kam_metrics(model, dataset,
                             save_peak_path='results/peak_kam_summary.csv',
                             save_frame_path='results/frame_kam_details.csv',
                             device='cpu'):
    """
    修改后函数同时生成两个文件:
...
peak_df, frame_df = extract_peak_kam_metrics(
    model,
    test_set,
    save_peak_path='results/peak_summary.csv',
    save_frame_path='results/frame_details.csv',
)
